...
export function AdminAutopilotLogSidebar() {
  ...
  const { active, logLines, state, busy } = useAutopilot();
  ...
  return (
    <aside
      className="autopilot-log-sidebar fixed bottom-0 left-0 right-0 z-20 flex h-[min(40dvh,16rem)] flex-col border-t border-[#1F2937] bg-[#0B0F14] shadow-2xl lg:bottom-0 lg:left-auto lg:right-0 lg:top-[57px] lg:h-auto lg:w-[min(100%,22rem)] lg:border-l lg:border-t-0"
      aria-label={t("logTitle")}
    >
      <header className="flex shrink-0 items-center justify-between border-b border-[#1F2937] px-3 py-2.5">

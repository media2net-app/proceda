...
const navItems: {
  ...
  { href: "/dashboard-admin/leads", key: "sidebarLeads", icon: "users" },
  { href: "/dashboard-admin/mail", key: "sidebarMail", icon: "mail", showUnread: true },
  {
    href: "/dashboard-admin/mail/sjablonen",
    key: "sidebarMailTemplates",
    icon: "mail",
  },
  {
    href: "/dashboard-admin/mail/log",
    key: "sidebarMailSentLog",
    icon: "mail",
  },
  {
    href: "/dashboard-admin/bellen",
    key: "sidebarColdCalling",
    icon: "calendar",
  },
...
function UsersIcon() {
  return (
    <svg className="h-4 w-4 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
      <path strokeLinecap="round" strokeLinejoin="round" d="M15 19.128a9.38 9.38 0 002.625.372 9.337 9.337 0 004.121-.952 4.125 4.125 0 00-7.533-2.493M15 19.128v-.003c0-1.113-.285-2.16-.786-3.07M15 19.128v.106A12.318 12.318 0 018.624 21c-2.331 0-4.512-.645-6.374-1.766l-.001-.109a6.375 6.375 0 0111.964-3.07M12 6.375a3.375 3.375 0 11-6.75 0 3.375 3.375 0 016.75 0zm8.25 2.25a2.625 2.625 0 11-5.25 0 2.625 2.625 0 015.25 0zM3.75 8.625a2.625 2.625 0 11-5.25 0 2.625 2.625 0 015.25 0z" />
    </svg>
  );
}
...
const navLinkClass = (active: boolean) =>
  `admin-nav-item flex items-center gap-2 rounded-md px-2 py-1.5 text-xs font-medium transition-colors ${
    active
...
export default function DashboardSidebar({
  ...
  const { verticalLabel, isAllBranches } = useAdminVertical();
  ...
  const sidebarContent = (
    <>
      <div className="flex h-12 shrink-0 items-center justify-between border-b border-[#EAECF0] px-3">
        <Link
          href="/"
          className="flex items-center gap-2 text-base font-bold tracking-tight text-[#101828]"
          onClick={onClose}
        >
          <span className="flex h-7 w-7 items-center justify-center rounded-full bg-[#7F56D9] text-xs font-bold text-white">
            P
          </span>
          Proceda
        </Link>
        {onClose ? (
          <button
            type="button"
            onClick={onClose}
            className="rounded-lg p-1.5 text-[#667085] hover:bg-[#F2F4F7] md:hidden"
      ...
      <div className="border-b border-[#EAECF0] px-2 py-2">
        <AdminVerticalSelector />
      </div>

      <nav className="scroll-touch flex flex-1 flex-col gap-px overflow-y-auto p-2">
        {navItems.map(({ href, key, icon, exact, showUnread }) => {
          ...
          const mailSuffix =
            key === "sidebarMail" && !isAllBranches ? ` · ${verticalLabel}` : "";
          return (
            <Link
              key={key}
              href={href}
            ...
            >
              ...
              {unread > 0 ? (
                <span className="rounded-full bg-[#7F56D9] px-1.5 py-0.5 text-[10px] font-bold text-white">
                  {unread}
                </span>
              ) : null}
            </Link>
        ...
        <div className="mt-0.5">
          ...
          >
            <NavIcon name="cube" />
            <span className="flex-1 truncate text-left">{t(productNav.key)}</span>
            <svg
          ...
          {productExpanded ? (
            <div className="ml-3 mt-px space-y-px border-l border-[#EAECF0] pl-1.5">
              {productNav.children.map(({ href, key, exact, isDemoApp }) => {
                ...
                return (
                  <Link
                    ...
                    onClick={onClose}
                    className={`${navLinkClass(isActive)} ${
                      isActive ? "admin-nav-product-active bg-[#F9F5FF] text-[#6941C6]" : ""
                    }`}
                  >
  ...
  return (
    <>
      {(onClose || forceCollapse) && (
        <button
          type="button"
          aria-hidden
          onClick={onClose}
          className={`fixed inset-0 z-[55] bg-[#101828]/40 transition-opacity ${forceCollapse ? (open ? "opacity-100 lg:block" : "pointer-events-none opacity-0") : `md:hidden ${open ? "opacity-100" : "pointer-events-none opacity-0"}`}`}
        />
      )}

      <aside
        className={`admin-sidebar safe-top safe-x fixed left-0 top-0 z-[60] flex h-full h-[100dvh] w-[min(100vw,14rem)] max-w-[85vw] flex-col border-r border-[#EAECF0] bg-white shadow-xl transition-transform duration-300 ease-out md:w-56 md:max-w-none ${
          open ? "translate-x-0" : "-translate-x-full"
        } ${forceCollapse ? "" : "md:translate-x-0"} md:flex`}
      >
        {sidebarContent}
      </aside>
    </>
